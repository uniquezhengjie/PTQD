...
n_bits_w = 8
n_bits_a = 8
save_dir = 'correct_data/imagenet_100steps_w{}a{}'.format(n_bits_w, n_bits_a)
...
def calculate_t_channel_error(t_tensor, error_tensor):
    ...
    np.save('correct_data/imagenet_100steps_w{}a{}/idx_bias.npy'.format(n_bits_w, n_bits_a), bias_arr)
    print(bias_table)
...
def get_kt_dict_imagenet(data_tensor, error_tensor, t_tensor):
    ...
    print(kt_dict)
    np.save('correct_data/imagenet_100steps_w{}a{}/kt.npy'.format(n_bits_w, n_bits_a), kt_dict, allow_pickle=True)

def get_t_residualerror_std_dict(t_tensor, data_tensor, error_tensor):
    '''
        need kt first for calculating residual error
    '''
    kt_dict = np.load('correct_data/imagenet_100steps_w{}a{}/kt.npy'.format(n_bits_w, n_bits_a), allow_pickle=True).item()
    t_std_dict = {}
    for i in range(len(t_tensor)):
        int_t = t_tensor[i].item()
    ...
    np.save("correct_data/imagenet_100steps_w{}a{}/t_std_dict.npy".format(n_bits_w, n_bits_a), t_std_dict, allow_pickle=True)


if __name__ == '__main__':
    data_error_t_list = torch.load('generated/data_error_t_w8a8_scale1.5_eta1.0_step100.pth', map_location='cpu')  ## replace error file here
    data_list = []
    error_list = [] 
    t_list = []
    ...
    print("finish get_kt_dict_imagenet")
    calculate_t_channel_error(t_tensor, error_tensor)
    print("finish calculate_t_channel_error")
    get_t_residualerror_std_dict(t_tensor, data_tensor, error_tensor)
    print("finish get_t_residualerror_std_dict")